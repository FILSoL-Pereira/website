...
type TeamMember = { src: string; linkedin: string };
...
export default function CarouselTeam({
  ...
  return (
    <section className={className ?? (isDark ? "bg-slate-950 py-6 w-full" : "bg-linear-to-b from-white via-gray-100 to-gray-200 py-6 w-full")}>
      <h2 className={`text-center text-3xl font-bold mb-4 ${isDark ? "text-white" : ""}`}>{title}</h2>
      <ImageCarousel
        images={team}
